...
describe("Mateus", () => {
  test("should display correct title", async () => {
    ...
    const foundTitleElement = titleElement.find(
      (element) => element?.textContent?.includes(expectedText),
    );
  ...
  test("mensagem de boas-vindas correta", () => {
    render(<Mateus />);
    const mensagemBoasVindas = screen.getByText(
      /Hey there! Welcome to my website/i,
    );
  ...
  test("renderização condicional de elementos específicos", () => {
    render(<Mateus />);
    const someConditionalElement = screen.queryByTestId(
      "someConditionalElement",
    );
